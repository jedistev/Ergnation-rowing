        ...
        <div class="col-lg-4 col-md-5">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h6> @if(Auth::user()->currentTeam) {{Auth::user()->currentTeam->name}} @else No team yet @endif
                    </h6>
                </div>
            </div>
        </div>
                ...
                <ul class="nav nav-pills custom-pills" id="pills-tab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" id="pills-timeline-tab" data-toggle="pill" href="#current-month"
                            role="tab" aria-controls="pills-timeline" aria-selected="true">{{ __('Setting')}}</a>
                    </li>
                </ul>
                        ...
                        <div class="card-body">
                            <form enctype="multipart/form-data" class="form-horizontal" method="POST"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{Auth::user()->firstname}}" class="form-control"
                                        name="firstname" id="firstname">
                                </div>
                                <div class="form-group">
                                    <label for="surname">{{ __('Surname')}}</label>
                                    <input type="text" value="{{Auth::user()->surname}}" class="form-control"
                                        name="surname" id="surname">
                                </div>
                                <div class="form-group">
                                    <label for="example-name">{{ __('Team')}}</label>
                                    <input disabled type="text"
                                        value="@if(Auth::user()->currentTeam) {{Auth::user()->currentTeam->name}} @else No team yet @endif"
                                        class="form-control" id="example-name">
                                </div>
                                <div class="form-group">
